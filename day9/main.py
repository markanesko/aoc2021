...
def find_basin(root = (-1,-1), basinmap = [], basinheight = -1, basinwidth = -1, area = 0):

    area += 1

    if root[0] == 0: #top row

        if root[1] == 0: #left egde
            right_pair = (root[0], root[1] + 1)
            
            if basinmap[right_pair[0]][right_pair[1]] != '*':
                area = find_basin(right_pair, basinmap, basinheight, basinwidth, area)
            
            bottom_pair = (root[0] + 1, root[1])
            
            if basinmap[bottom_pair[0]][bottom_pair[1]] != '*':
                area = find_basin(bottom_pair, basinmap, basinheight, basinwidth, area)
    
        if root[1] == basinwidth - 1: #right edge
            left_pair = (root[0], root[1] - 1)
            
            if basinmap[left_pair[0]][left_pair[1]] != '*':
                area = find_basin(left_pair, basinmap, basinheight, basinwidth, area)
    ...
    if root[0] == basinheight - 1: #bottom row
        print('bottomrow')
        ...
        if root[1] == 0: #left egde
            ...
            top_pair = (root[0] - 1, root[1])
            
            if basinmap[top_pair[0]][top_pair[1]] != '*':
                area = find_basin(top_pair, basinmap, basinheight, basinwidth, area)
